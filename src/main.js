...
function handleLoadMore() {
  ...
  refs.cssLoading.classList.remove('hidden');
  fetchPictures()
    .then(() => buttonService.enable())
    .finally(() => refs.cssLoading.classList.add('hidden'));
}

function fetchPictures() {
  return pixabayService.getPictures(queryParams)
    .then(({ data }) => {
      ...
      if (data.totalHits <= queryParams.perPage * queryParams.page) {
        iziToast.info({
          position: 'topRight',
          title: 'Info',
          message: `Hooray! We found ${data.totalHits} images.`,
